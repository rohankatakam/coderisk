...
func NewRegistry(neo4j *graph.Client, postgres *database.Client) *Registry {
	return &Registry{
		neo4j:    neo4j,
		postgres: postgres,
...
func (r *Registry) CalculatePhase1(ctx context.Context, repoID, filePath string) (*Phase1Result, error) {
	...
	go func() {
		...
		// Calculate coupling
		coupling, err := CalculateCoupling(ctx, r.neo4j, repoID, filePath)
		if err != nil {
		...
		// Calculate co-change
		coChange, err := CalculateCoChange(ctx, r.neo4j, repoID, filePath)
		if err != nil {
		...
		testRatio, err := CalculateTestRatio(ctx, r.neo4j, repoID, filePath)
		if err != nil {
